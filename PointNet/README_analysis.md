...
- 参数量: 原论文和官方 README 未给出统一的 LiDAR 语义分割参数量；分类/part segmentation/scene parsing 配置不同，不能混用。
- 计算耗时: 原论文未报告自动驾驶 LiDAR scan 级推理耗时；官方代码也没有提供可直接横比的 SemanticKITTI FPS。

## 论文与代码地址

- 论文地址: https://arxiv.org/abs/1612.00593
- GitHub 仓库: https://github.com/charlesq34/pointnet

## 核心创新代码块

```python
# PointNet 核心结构伪代码
point_feat = shared_mlp(points)          # per-point MLP
global_feat = max_pool(point_feat)       # symmetric function
seg_feat = concat(point_feat, global_feat.repeat(N, 1))
logits = shared_mlp(seg_feat)
```

## 使用方法描述

使用时把点云作为无序集合输入，通过共享 MLP 和 max pooling 得到 permutation-invariant 表示；适合作基础理论和小规模 point set baseline。
