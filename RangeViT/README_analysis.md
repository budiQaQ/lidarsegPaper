...
- 参数量: 本地 README/论文整理中未确认统一参数量；ViT backbone 大小、patch size、decoder/refiner 会改变规模。
- 计算耗时: 当前整理材料未找到可直接横比的 SemanticKITTI 单帧 latency；评估时应单独统计 stem、ViT encoder、decoder/refiner 和回投影。
