...
- 参数量: CENet 代码 `modules/trainer.py` 会打印 trainable parameters，但当前本地环境未安装 PyTorch，未运行统计；论文主要强调参数/算子友好的普通卷积组合，而不是给出单一参数量表。
- 计算耗时: 官方 `assert/finalspeed.md` 中 RTX 3080 口径，`64x2048` RES+Hardswish 约 37.79 FPS，CNN 约 26.18 ms、KNN 约 0.28 ms；`64x1024` 约 67.97 FPS；`64x512` 约 84.91 FPS。

## 论文与代码地址

- 论文地址: https://arxiv.org/abs/2207.12691
- GitHub 仓库: https://github.com/huixiancheng/CENet

## 核心创新代码块

```python
# CENet/code/tasks/semantic/modules/trainer.py 的训练逻辑核心
# main loss = weighted CE + Lovasz + boundary loss + auxiliary heads
loss = loss_main_ce + lambda_lovasz * loss_main_lovasz + lambda_boundary * loss_boundary
loss = loss + sum(auxiliary_losses)
```

## 使用方法描述

使用时以官方 `config/arch/*.yaml` 训练 CENet；重点保留 auxiliary heads、boundary loss 和 FID-like interpolation decoding，单独去掉这些模块会影响论文中的增益。
