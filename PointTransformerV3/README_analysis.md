...
- 参数量: 官方 README model zoo 未列出统一参数量；PTv3 m1/base、PPT 预训练和数据集配置会改变规模。
- 计算耗时: 论文整理中记录相比 PTv2 推理延迟从约 48 ms 降到约 44 ms，显存从 1.7G 降到 1.2G；这是 3D point Transformer 口径，不是 range-view student latency。

## 论文与代码地址

- 论文地址: https://arxiv.org/abs/2312.10035
- GitHub 仓库: https://github.com/Pointcept/PointTransformerV3

## 核心创新代码块

```python
# PointTransformerV3 独立使用方式
from model import PointTransformerV3
model = PointTransformerV3(enable_flash=True)
encoded_points = model(input_dict)  # input_dict 按 README/model.py 定义组织
```

## 使用方法描述

使用时推荐走 Pointcept 配置训练；若只借鉴模型，可复制 `model.py` 和 `serialization/`，重点保留 serialization 与 FlashAttention 设置。
