...
- 参数量: 官方 README 未列出统一参数量；依赖 pointops2 和具体 indoor segmentation 配置。
- 计算耗时: 当前本地材料未找到可与 LiDAR range-view 方法横比的单帧 latency；稀疏远处 key sampling 主要用于降低长程 attention 成本。

## 论文与代码地址

- 论文地址: https://arxiv.org/abs/2203.14508
- GitHub 仓库: https://github.com/dvlab-research/Stratified-Transformer

## 核心创新代码块

```python
# Stratified Transformer 核心采样伪代码
K_dense = points_in_local_window(query)
K_sparse = fps(points).within_large_window(query)
keys = union(K_dense, K_sparse)
out = multi_head_attention(query, keys, values)
```

## 使用方法描述

使用时需编译 `lib/pointops2`；它主要作为 3D point Transformer 的 long-range context 参考，不是直接的 range-view 实时 baseline。
