...
- 参数量: 论文对比表给出约 3.8M 参数。
- 计算耗时: PandaSet validation 口径 runtime 约 38 ms；相比 RangeFormer 23.7M/54 ms 更轻，但仍比 FIDNet/CENet 的极简 CNN pipeline 慢一些。

## 论文与代码地址

- 论文地址: https://openaccess.thecvf.com/content/WACV2026/papers/Mosco_Revisiting_Retentive_Networks_for_Fast_Range-View_3D_LiDAR_Semantic_Segmentation_WACV_2026_paper.pdf
- GitHub 仓库: https://github.com/SiMoM0/RangeRet

## 核心创新代码块

```python
# RangeRet/code/network/retnet.py
...
使用时按数据集选择 `config/RangeRet-*.yaml`；CiR 会显式建模 range image 水平方向 360 度环绕，适合作轻量 range-view student。
